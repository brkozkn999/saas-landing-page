...
const AccordionItem = ({ question, answer }: { question: string, answer: string }) => {
  ...
  return (
    <div key={question}>
      <div className="flex items-center py-7 border-b border-white/30" onClick={() => setIsOpen(!isOpen)}>
        <span className="flex-1 text-lg font-bold">{question}</span>
        <PlusIcon className={isOpen ? "-rotate-45 transition-all ease-in-out duration-300" : "transition-all ease-in-out duration-200"} />
      </div>
      <AnimatePresence>
        {isOpen && (
          <motion.div
